    ...
    <div className="p-6">
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
          ...
          <p className="mt-1 text-2xl font-semibold text-emerald-600">{summary.done}</p>
        </div>
        <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
          <p className="text-xs text-gray-500">Overdue</p>
          <p className="mt-1 text-2xl font-semibold text-red-600">{summary.overdue}</p>
        </div>
      </div>

      <h2 className="mt-8 mb-3 text-sm font-semibold text-gray-700">All Sprint Tasks</h2>
      <DataTable
        columns={columns}
        data={tasks}
        isLoading={tasksLoading}
        caption="Sprint task summary"
      />
    </div>
  );
}
